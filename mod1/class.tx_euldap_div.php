			...
			if ($memberOf == 'posixgroup') {
				...
				if (is_array($ldapres[$memberOf])) {
					while ($k < count($ldapres[$memberOf])) {
						...
						$ldapres[$memberOf] = array_merge((array)$ldapres[$memberOf], (array)$groupies[$memberOf]);
						// Version 2012-05-21/wob: uniq, damit das array nicht ins unendliche waechst
						$ldapres[$memberOf] = array_unique($ldapres[$memberOf]);

						// $ldapres[$memberOf] = array_merge($ldapres[$memberOf], $groupies[$memberOf]);
						// <--- 2014-04-26/wob: indirekte Gruppenmitgliedschaften ---
